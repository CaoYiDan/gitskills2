...
-(void)createModel:(TCOrderModel*)model{
    ...
    if(!isNull(model.shipingTotalprice)&&!isEmptyString(model.shipingTotalprice)){
        ...
        [_totalPrice setLabelText:[NSString stringWithFormat:@"%.2f",model.actualTotalprice]];
    }
    
    //发货时间
    [_deliverDate setLabelText:[NSString stringWithFormat:@"发货时间:%@",model.createDateStr]];
    //吨
    if ([model.sizeUnit isEqualToString:@"吨"]) {
        _weight.type=0;
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f",model.tenderWeight]];
        
    }else{
        //单位不是吨
        _weight.type=4;
        
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f %@",model.tenderWeight,model.sizeUnit]];
    }
//    //重量
//    [_weight setLabelText:[NSString stringWithFormat:@"%.1f",model.tenderWeight]];
    //单价
    [_unitPrice setLabelText:[NSString stringWithFormat:@"%.2f 元/吨",model.offerPrice]];
    //要求
    _requirement.text=model.remarks;
    
//    //路径规划
//    [self loacationMapWithStart:model.outProvince startName:model.outCity andEndCity:model.receiveProvince endName:model.receiveCity];
//}
}
-(void)createUI{
