...
-(void)setOrderModel:(TCOrderModel *)orderModel
{
    ...
    if ([orderModel.sizeUnit isEqualToString:@"吨"]) {
        _weight.type=0;
        //重量
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f",orderModel.actualNum]];
    }else{
        _weight.type=4;
        //重量
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f%@",orderModel.actualNum,orderModel.sizeUnit]];
    }
    //司机
    [_driver setLabelText:[NSString stringWithFormat:@"%@",orderModel.driverName]];
    NSLog(@"%@",orderModel.outDate);
    //发货的时间
