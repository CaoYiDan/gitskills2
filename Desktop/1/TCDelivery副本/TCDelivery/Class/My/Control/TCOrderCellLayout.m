    ...
    NSMutableAttributedString *goodsWeightAttbuiteText = [NSMutableAttributedString new];
    {
        if (_switchType==SwitchTypeMyPublishList) {
            ...
            if ([_model.weightUnit isEqualToString:@"吨"]) {
                [goodsWeightAttbuiteText appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %.1f ",_model.goodsWeight] attributes:nil]];
                ...
                [goodsWeightAttbuiteText appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %.1f %@",_model.goodsWeight,_model.weightUnit] attributes:nil]];
            }
        }else{
            ...
            if ([_model.sizeUnit isEqualToString:@"吨"]) {
                ...
                [goodsWeightAttbuiteText appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %.1f ",_model.tenderWeight] attributes:nil]];
                //重量
                UIImage *image = [UIImage imageNamed:@"fh_dd_xq_an1"];
                image = [UIImage imageWithCGImage:image.CGImage scale:2 orientation:UIImageOrientationUp];
                ...
                //若重量单位不是吨，则直接拼接后台返回的重量单位
                [goodsWeightAttbuiteText appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@" %.1f %@",_model.tenderWeight,_model.sizeUnit] attributes:nil]];
            }
        }
    }
