...
-(void)setModel:(TCOrderModel *)model{
    ...
    if ([model.weightUnit isEqualToString:@"吨"]) {
        _weight.type=0;
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f",model.goodsWeight]];

    }else{
        //单位不是吨
        _weight.type=4;
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f%@",model.goodsWeight,model.weightUnit]];
    }
     //体积
