...
-(void)setOrderModel:(TCOrderModel *)orderModel
{
    ...
    if ([orderModel.sizeUnit isEqualToString:@"吨"]) {
        _weight.type=0;
        //重量
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f",orderModel.tenderWeight]];
    }else{
        _weight.type=4;
        //重量
        [_weight setLabelText:[NSString stringWithFormat:@"%.1f%@",orderModel.tenderWeight,orderModel.sizeUnit]];
    }
    //长度
