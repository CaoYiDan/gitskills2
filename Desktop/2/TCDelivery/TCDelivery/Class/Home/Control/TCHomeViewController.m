...
{   page=1;
    ...
    [[HttpRequest sharedClient]httpRequestPOST:kHomeUrl parameters:dict progress:nil sucess:^(NSURLSessionDataTask *task, id responseObject, ResponseObject *obj){
        ...
        for (NSDictionary*dic in responseObject[@"data"][@"advertDtoList"]) {
            ...
            [banner addObject:[NSString stringWithFormat:@"http://www.tiancheng100y.com/tcwl-image-server%@",dic[@"imgUrl"]]];
            NSLog(@"%@",[NSString stringWithFormat:@"http://www.tiancheng100y.com/tcwl-image-server%@",dic[@"imgUrl"]]);
        }
        //赋值给headerView---轮播图
